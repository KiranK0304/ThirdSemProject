...
class TestApplicationNotifications:
    def setup_method(self):
        self.client = APIClient()
        self.employer_user = User.objects.create_user(email="employer-notif@example.com", password="Password123!")
        self.employer_profile = EmployerProfile.objects.create(
            user=self.employer_user,
            company_name="Example Co",
            verification_status=VerificationStatus.APPROVED,
        )
        self.seeker_user = User.objects.create_user(email="seeker-notif@example.com", password="Password123!")
    ...
    def test_application_submission_notifies_employer(self):
        login_resp = self.client.post(
            reverse("auth_api:login"),
            {"email": "seeker-notif@example.com", "password": "Password123!"},
            format="json",
        )
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {login_resp.data['access']}")

        response = self.client.post(
            reverse("applications_api:job-apply", kwargs={"job_id": self.job.pk}),
            {"cover_letter": "I would love to join."},
            format="json",
        )
    ...
    def test_application_status_change_notifies_seeker_once(self):
        ...
        login_resp = self.client.post(
            reverse("auth_api:login"),
            {"email": "employer-notif@example.com", "password": "Password123!"},
            format="json",
        )
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {login_resp.data['access']}")
